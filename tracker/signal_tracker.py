...
class SignalTracker:
    ...
    Responsibilities:
    - Record new signals.
    - Tag every new signal with the active strategy version.
    ...
    def __init__(
        ...
        # Migrate an existing Signals sheet once the tracker connects.
        # This preserves all historical data and labels it as v1.
        self.sheets.ensure_strategy_version_column()
    ...
    def build_signal(
        ...
        return {
            ...
            "RSI": round(pick.rsi, 2),
            "MACD": round(pick.macd, 2),
    ...
    def record_signals(
        ...
        for rank, pick in enumerate(picks, start=1):
            ...
            if self.sheets.signal_exists(signal_id):
                print(
                    f"Signal already exists: {signal_id}"
                )
                continue

            self.sheets.add_signal(signal)

            print(
                f"Recorded signal: {signal_id} "
                f"[{STRATEGY_VERSION}]"
            )
    ...
    def update_open_trades(self) -> None:
        ...
        open_signals = self.sheets.get_open_signals()
        ...
        print(
            f"Open signals to check: {len(open_signals)}"
        )
        ...
        for signal in open_signals:
            try:
                ...
                print(
                    f"Could not update {signal_id}: {exc}"
                )
    ...
    def _update_single_signal(
        ...
        signal_date = date.fromisoformat(
            str(signal["Signal Date"])[:10]
        )

        entry = float(signal["Entry Price"])
        target = float(signal["Target Price"])
        stop_loss = float(signal["Stop Loss"])
        today = self.now_ist().date()

        market_rows = self.market_data.get_data_after_signal(
            symbol=symbol,
            signal_date=signal_date,
            end_date=today,
        )
        ...
        if not market_rows:
            self.sheets.update_signal(
                signal_id,
                {
                    "Last Checked": self.now_ist().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                },
            )
            ...
            print(
                f"{signal_id}: No post-signal market data yet."
            )
            return

        for market_row in market_rows:

            observation_date = market_row["date"]
            high = market_row.get("high")
            low = market_row.get("low")
            ...
            price_log = {
                ...
                "Status": status,
                "Data Source": "GOOGLEFINANCE",
            }

            self._append_price_log_if_new(price_log)
            ...
            if status in (
                ...
                updates = {
                    "Status": status,
                    "Exit Date": observation_date.isoformat(),
                    "Days Held": days_held,
                ...
                if exit_price is not None:
                    ...
                    updates["Exit Price"] = round(
                        exit_price,
                        2,
                    )
                    updates["Return %"] = round(
                        return_pct,
                        2,
                    )
                    updates["Current Price"] = round(
                        exit_price,
                        2,
                    )
                    updates["Current Return %"] = round(
                        return_pct,
                        2,
                    )
                else:
                ...
                print(
                    f"{signal_id}: {status} on {observation_date}"
                )
                return
    ...
    def _append_price_log_if_new(
        ...
        for record in records:
            ...
            if (
                ...
                and existing_date == observation_date
            ):
                print(
                    f"Price log already exists: "
                    f"{signal_id} / {observation_date}"
                )
                return

        self.sheets.add_price_log(price_log)
