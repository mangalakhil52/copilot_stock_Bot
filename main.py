...
from telegram_notifier import send_telegram_alert, format_telegram_alert
        ...
        rows.append(
            {
                ...
                "volume_ratio": f"{pick.volume / pick.avg_volume:.2f}x" if pick.avg_volume and pick.avg_volume > 0 else "N/A",
...
def print_picks(picks: List[SwingPick]) -> None:
    ...
    for index, pick in enumerate(picks, start=1):
        ...
        print(f"   Close: {pick.close:.2f}  Entry: {pick.entry:.2f}  Score: {pick.score:.1f}  Change: {pick.percent_change:.2f}%")
        print(f"   EMA20: {pick.ema20:.2f}  EMA50: {pick.ema50:.2f}  SMA20: {pick.sma20:.2f}  SMA50: {pick.sma50:.2f}")
        avg_volume_text = f"{pick.avg_volume:.0f}" if pick.avg_volume and pick.avg_volume > 0 else "N/A"
        print(f"   Volume: {pick.volume:.0f}  Avg20: {avg_volume_text}  RSI: {pick.rsi:.1f}  MACD: {pick.macd:.2f}")
        print(f"   Industry: {pick.industry or 'N/A'}")
        print(f"   Target: {pick.target:.2f}  Stop Loss: {pick.stop_loss:.2f}  Hold: {pick.holding_period}  Risk: {pick.risk_pct:.2f}%  Reward: {pick.reward_pct:.2f}%")
...
def run_scan(config: Optional[Dict[str, Any]] = None) -> List[SwingPick]:
    ...
    rows = client.scan_candidates(scan_clause=scan_clause, column_clause=column_clause, max_candidates=max_candidates)
    picks = build_swing_picks(rows, top_n=top_n, exclude_keywords=exclude_keywords)
    return picks
...
def main() -> int:
    ...
    run_parser = subparsers.add_parser("run", help="Run scan and optionally publish alerts from config.")
    ...
    run_parser.add_argument("--skip-telegram", action="store_true", help="Do not send Telegram alerts even if config is provided.")
